...
document.addEventListener("alpine:init", () => {
    ...
    Alpine.data("dictation", (options = {}) => ({
        ...
        base: "",
        /** What the engine has committed to since — no longer up for revision. */
        settled: "",
        ...
        start() {
            ...
            if (this.listening && navigator.vibrate) navigator.vibrate(8);
        },
         ...
         * box to `$wire.body`, so this is the end the binding reads from and
         * the box follows. Whitespace is tidied on the spoken half only —
         * engines vary on the leading space and none of them emit newlines,
         * while the typed half is somebody's own line breaks.
         */
        write(spoken) {
            const said = spoken.replace(/\s+/g, " ").trimStart();
            const gap = this.base && !this.base.endsWith(" ") ? " " : "";

            this.$wire.body = (this.base + gap + said).slice(0, this.max);
        },
