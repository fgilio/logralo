...
# environment.sh creates the file when it writes .env from the profile. Cover
# the case where .env already existed and the file did not.
[ -f database/database.sqlite ] || touch database/database.sqlite

# The profile ships APP_KEY empty and environment.sh's key:generate runs before
# vendor/ exists, so it cannot have succeeded. Generate it here, where artisan
# is available. Anything that touches encryption fails without it.
