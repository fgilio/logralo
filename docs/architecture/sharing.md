...
"The same way everywhere" includes a first day. The card shipped once hiding a streak of 1, on the theory that a run of one is not a run; what it did in practice was send a bare date into the chat every time somebody came back after a gap — the share most worth making — while the feed card the share button sits on went on showing the 1. A number missing from one surface and present on the other reads as the flame being broken, which is exactly how it was reported. `MarkEntry::shareCard()` passes the count through as it is, and only the recap, which counts no days of its own, leaves it null.
